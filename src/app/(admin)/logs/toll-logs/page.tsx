...
export default function TollLogsPage() {
  ...
  const [selectedLog, setSelectedLog] = useState<TollLog & Record<string, unknown> | null>(null);
  ...
  const fetchLogs = async (params?: { page?: number; search?: string }) => {
    ...
    setError(null);
    try {
      const response = await tollLogService.getTollLogs({
        page: params?.page || 1,
        limit: 25,
        search: params?.search,
      });
      
      console.log('Toll Log fetch response:', response);
      
      let logsData: TollLog[] = [];
      let paginationData: {
        page: number;
        pageSize: number;
        pageCount: number;
      ...
      if (response.data) {
        if (Array.isArray(response.data)) {
          ...
          const strapiResponse = response.data as unknown as { data: TollLog[]; meta?: { pagination?: typeof paginationData } };
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch toll logs';
      setError(errorMessage);
      console.error('Error fetching toll logs:', err);
  ...
  const handleDeleteLog = (log: TollLog & Record<string, unknown>) => {
    setSelectedLog(log);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await tollLogService.deleteTollLog(selectedLog.documentId);
      showSuccessToast('Toll log deleted successfully!');
      fetchLogs({ page: pagination?.page || 1, search: searchQuery || undefined });
      setIsDeleteModalOpen(false);
      setSelectedLog(null);
    } catch (error) {
      console.error('Error deleting toll log:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to delete toll log';
      showErrorToast(errorMessage);
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <div className="grid grid-cols-12 gap-4 md:gap-6">
      <div className="col-span-12">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Toll Logs</h1>
          <p className="text-gray-600 dark:text-gray-400">View and manage all toll logs</p>
        </div>
      ...
      <div className="col-span-12">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Toll Logs List</h2>
          </div>
          ...
          <div className="p-6">
            ...
            {isLoading && (
              <div className="flex justify-center items-center py-8">
                ...
                <span className="ml-2 text-gray-600 dark:text-gray-400">Loading toll logs...</span>
              </div>
            )}
            ...
            {!isLoading && logs.length === 0 && !error && (
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No toll logs found</h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">No toll logs are available at the moment.</p>
              </div>
            )}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {logs.map((log: TollLog & Record<string, unknown>) => {
                      const logData = log as Record<string, unknown>;
                      return (
                        <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900 dark:text-white">
                              ₹{String(logData.totalTollAmount || 'N/A')}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white">
                              {String(logData.numberOfTollCrosses || 'N/A')}
                            </div>
                          </td>
      ...
      <ConfirmationModal
        ...
        title="Delete Toll Log"
        message={`Are you sure you want to delete this toll log? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"

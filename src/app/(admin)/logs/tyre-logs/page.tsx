...
export default function TyreLogsPage() {
  ...
  const [selectedLog, setSelectedLog] = useState<TyreLog & Record<string, unknown> | null>(null);
  ...
  const fetchLogs = async (params?: { page?: number; search?: string }) => {
    ...
    setError(null);
    try {
      const response = await tyreLogService.getTyreLogs({
        page: params?.page || 1,
        limit: 25,
        search: params?.search,
      });
      
      console.log('Tyre Log fetch response:', response);
      
      let logsData: TyreLog[] = [];
      let paginationData: {
        page: number;
        pageSize: number;
        pageCount: number;
      ...
      if (response.data) {
        if (Array.isArray(response.data)) {
          ...
          const strapiResponse = response.data as unknown as { data: TyreLog[]; meta?: { pagination?: typeof paginationData } };
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch tyre logs';
      setError(errorMessage);
      console.error('Error fetching tyre logs:', err);
  ...
  const handleDeleteLog = (log: TyreLog & Record<string, unknown>) => {
    setSelectedLog(log as TyreLog & Record<string, unknown>);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await tyreLogService.deleteTyreLog(selectedLog.documentId);
      showSuccessToast('Tyre log deleted successfully!');
      fetchLogs({ page: pagination?.page || 1, search: searchQuery || undefined });
      setIsDeleteModalOpen(false);
      setSelectedLog(null);
    } catch (error) {
      console.error('Error deleting tyre log:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to delete tyre log';
      showErrorToast(errorMessage);
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <div className="grid grid-cols-12 gap-4 md:gap-6">
      <div className="col-span-12">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Tyre Logs</h1>
          <p className="text-gray-600 dark:text-gray-400">View and manage all tyre logs</p>
        </div>
      ...
      <div className="col-span-12">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Tyre Logs List</h2>
          </div>
          ...
          <div className="p-6">
            ...
            {isLoading && (
              <div className="flex justify-center items-center py-8">
                ...
                <span className="ml-2 text-gray-600 dark:text-gray-400">Loading tyre logs...</span>
              </div>
            )}
            ...
            {!isLoading && logs.length === 0 && !error && (
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No tyre logs found</h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">No tyre logs are available at the moment.</p>
              </div>
            )}
                    ...
                    {logs.map((log: TyreLog & Record<string, unknown>) => {
                      ...
                      return (
                        <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white">
                              {String(logData.fitmentKM || 'N/A')} KM
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white">
                              {String(logData.brand || 'N/A')}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white capitalize">
                              {String(logData.tyrePosition || 'N/A')}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white">
                              {String(logData.removalTyreNumber || 'N/A')}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white">
                              {String(logData.removalKM || 'N/A')} KM
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900 dark:text-white">
                              {String(logData.removalReason || 'N/A')}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900 dark:text-white">
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to delete this tyre log? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"

...
import type { Trip, TripUpdateRequest, TripStatus } from '@/store/api/types';
...
export default function TripEditForm({ trip, onSuccess, onCancel }: TripEditFormProps) {
  ...
  const handleInputChange = (field: keyof TripUpdateRequest, value: string | null) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedTrip = await updateTrip(formData.id, formData);
      if (updatedTrip) {

...
const GarageLogCreateModal: React.FC<GarageLogCreateModalProps> = ({
  ...
  const [formData, setFormData] = useState<GarageLogFormData>({
    ...
    invoiceTime: new Date().toTimeString().slice(0, 5), // HH:MM format
    particular: '',
  ...
  //   handleInputChange('invoiceDoc', file);
  // };
    ...
    try {
      ...
      if (formData.invoiceDoc) {
        try {
          ...
          // Strapi v4 upload endpoint expects 'files' (plural) and returns an array
          // The endpoint is /api/upload (not /upload)
          const uploadResponse = await api.post<Array<{ id?: number; documentId?: string }>>('/api/upload', uploadFormData);

          // Extract file ID from response (Strapi returns an array)
          if (uploadResponse.data && Array.isArray(uploadResponse.data) && uploadResponse.data.length > 0) {
            const uploadedFile = uploadResponse.data[0];
            // Use documentId if available (Strapi v5), otherwise use id (Strapi v4)
            const fileId = uploadedFile.documentId || uploadedFile.id;
            if (fileId) {
              logData.invoiceDoc = fileId;
              console.log('File uploaded successfully, file ID:', fileId);
            } else {
              console.warn('File uploaded but no ID found in response:', uploadResponse.data);
            }
          } else {
            console.warn('File upload response format unexpected:', uploadResponse);
          }
        } catch (uploadError) {
            ...
            {/* <div className="col-span-2">
              ...
              <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg dark:border-gray-600">
                <div className="space-y-1 text-center">
                  ...
                  >
                    <path
                      d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-4h12m-4 4H12m16 0v12a4 4 0 01-4 4H12a4 4 0 01-4-4V36"
                      strokeWidth={2}
                      strokeLinecap="round"
                  ...
                  {formData.invoiceDoc && (
                    <p className="text-sm text-gray-700 dark:text-gray-300 mt-2">
                      Selected: {formData.invoiceDoc.name}
                    </p>
                  )}
                </div>
              </div>

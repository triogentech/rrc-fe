...
export default function TripEditModal({ isOpen, onClose, trip, onSuccess }: TripEditModalProps) {
  useEffect(() => {
    if (isOpen) {
        ...
        <div className="relative w-full max-w-4xl mx-auto">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl transform transition-all w-full">
            <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Edit Trip: {trip.tripNumber}</h3>
            </div>
            <div className="p-6">
              <TripEditForm trip={trip} onSuccess={handleSuccess} onCancel={onClose} />
            </div>
          </div>
        </div>
      </div>

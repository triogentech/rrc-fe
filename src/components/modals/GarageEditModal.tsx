...
const GarageEditModal: React.FC<GarageEditModalProps> = ({
  isOpen,
  onClose,
  garage,
  onSuccess,
  ...
  // Fetch cities and initialize form when modal opens or garage changes
  useEffect(() => {
    if (isOpen) {
      fetchCities();
      if (garage) {
        const cityId = typeof garage.city === 'object' && garage.city ? garage.city.documentId : 
                      typeof garage.city === 'string' ? garage.city : '';
        setFormData({
          name: garage.name || '',
          isActive: garage.isActive ?? true,
          city: cityId,
        });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await garageService.updateGarage(garage.documentId, garageData);
      showSuccessToast(`Garage "${formData.name}" updated successfully!`);
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Failed to update garage:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to update garage';
...
export default GarageEditModal;
